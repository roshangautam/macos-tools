...
def check_brew_exists() -> bool:
    ...
    try:
        result = subprocess.run(
            ["which", "brew"], capture_output=True, text=True, check=False
        )
...
def run_brew_command(
    command: List[str], streaming: bool = False
) -> Tuple[int, str, str]:
...
def parse_brew_dir_sizes() -> Dict[str, int]:
    ...
    sizes = {
        ...
        "Core": (
            get_dir_size(os.path.join(base_dir, "Homebrew"))
            if os.path.exists(os.path.join(base_dir, "Homebrew"))
            else 0
        ),
        "Cask": (
            get_dir_size(os.path.join(base_dir, "Caskroom"))
            if os.path.exists(os.path.join(base_dir, "Caskroom"))
            else 0
        ),
    }
...
def brew_cleanup(dry_run, json_output):
    ...
    click.echo(
        f"{'Simulating cleanup' if dry_run else 'Cleaning up'} Homebrew files..."
    )
...
def brew_doctor(json_output):
    ...
    if json_output:
        result = {
            ...
            "has_issues": bool(re.search(r"warning", stdout + stderr, re.IGNORECASE))
            or returncode != 0,
        }
        click.echo(json.dumps(result, indent=2))
...
def brew_size(json_output):
    ...
    if json_output:
        ...
        for category in categories:
            size = sizes[category]
            click.echo(
                f"| {category:{category_width}} | {format_size(size):{size_width}} |"
            )

        # Separator before total
        click.echo(f"{'-' * (category_width + size_width + 7)}")
        click.echo(
            f"| {'Total':{category_width}} | {format_size(sizes['Total']):{size_width}} |"
        )
        click.echo(f"{'-' * (category_width + size_width + 7)}")
