...
@system.command()
@click.option(
    "--force", is_flag=True, help="Actually perform cleanup instead of simulating"
)
@click.option("--caches", is_flag=True, help="Clean ~/Library/Caches")
@click.option("--logs", is_flag=True, help="Clean ~/Library/Logs")
@click.option(
    "--app-caches", is_flag=True, help="Clean ~/Library/Application Support/Caches"
)
@click.option("--tmp", is_flag=True, help="Clean /tmp directory")
@click.option(
    "--var-folders", is_flag=True, help="Clean /private/var/folders (use with caution)"
)
@click.option(
    "--all", "clean_all", is_flag=True, help="Clean all temporary directories"
)
def cleanup_temp(force, caches, logs, app_caches, tmp, var_folders, clean_all):
            ...
            if force:
                ...
                if dir_info["safe_to_remove"]:
                    ...
                    with click.progressbar(
                        os.listdir(dir_path),
                        label=f"Cleaning {dir_info['description']}",
                    ) as items:
                        for item in items:
                            ...
                            try:
                                ...
                                time.sleep(
                                    0.01
                                )  # Small delay for progress bar visibility
                    ...
                    for root, dirs, files in os.walk(dir_path, topdown=True):
                        ...
                        for pattern in exclude_patterns:
                            dirs[:] = [
                                d
                                for d in dirs
                                if not Path(os.path.join(root, d)).match(pattern)
                            ]
                            files[:] = [
                                f
                                for f in files
                                if not Path(os.path.join(root, f)).match(pattern)
                            ]
                    ...
                    with click.progressbar(
                        items, label=f"Cleaning {dir_info['description']}"
                    ) as progress_items:
    ...
    if force:
        ...
        click.echo(
            "Note: This was just a simulation. Add --force to actually perform cleanup."
        )
