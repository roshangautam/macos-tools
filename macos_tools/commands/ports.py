...
@click.option("--mail", is_flag=True, help=f"Include common mail ports: {', '.join(str(p) for p in COMMON_PORTS['mail'])}")
@click.option("--all-common", "-a", is_flag=True, help="Include all common ports from all categories")
@click.option("--json", "json_output", is_flag=True, help="Output in JSON format")
def list_ports(port: Tuple[int], web: bool, db: bool, dev: bool, mail: bool, all_common: bool, json_output: bool):
    ...
    if json_output:
        ...
        for port_num, processes in results.items():
            ...
            for i, proc in enumerate(processes, 1):
                click.echo(f"  {i}. PID: {proc['pid']}, Command: {proc['command']}, User: {proc['user']}, Protocol: {proc['protocol']}, State: {proc['state']}")
...
@click.option("--signal", "-s", type=str, default=None, help="Specify signal to send (e.g., 9, 15, TERM, KILL)")
def kill_port(port: int, force: bool, yes: bool, signal: Optional[str]):
    ...
    for proc in processes:
        click.echo(f"  - PID: {proc['pid']}, Command: {proc['command']}, User: {proc['user']}")
...
@click.option("--host", "-h", default="localhost", help="Host to scan (default: localhost)")
@click.option("--timeout", "-t", type=float, default=0.5, help="Connection timeout in seconds (default: 0.5)")
@click.option("--json", "json_output", is_flag=True, help="Output in JSON format")
def scan_ports(start: int, end: int, common: bool, open_only: bool, host: str, timeout: float, json_output: bool):
    """Scan a range of ports to determine which are open or in use.
    
    Defaults to scanning ports 8000-9000 on localhost.
    """
    if start < 1 or end > 65535 or start > end:
        click.echo("Invalid port range. Ports must be between 1 and 65535, and start must be less than or equal to end.", err=True)
        return
    ...
    if json_output:
        ...
        for port, info in sorted(results.items()):
            ...
            if info["open"]:
                if processes:
                    proc_info = ", ".join(f"{p['command']} (PID: {p['pid']})" for p in processes)
                    click.echo(f"Port {port}: {status} - Used by {proc_info}")
                else:
                    click.echo(f"Port {port}: {status} - No process found (port might be in TIME_WAIT state)")
            elif not open_only:
